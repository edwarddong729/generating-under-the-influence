...
STEL (how? manually?)
BERT, etc. -> topline, style+topic
punctuation similarity 


NLTK POS tags 
    my thoughts:
        adverb to verb ratio (what if 0? consider for all ratio-ed cases)
        adjective to noun ratio 
        verb to noun ratio
        prepositions normalized by total word count
        pronoun / noun ratio

    evaluate using explicit clustering metrics OR variance calculation OR over means OR difference of means
    pick top 32 for embeddings
    or just go right into PCA, pca will pick the relevant metrics for you 



cased characters??

start out by showing why you use certain embeddings and not others, 
rationale for discarding the ones that don't separate out the author styles 
looking for the right measurement tool 
this doesn't work, we don't want this one, etc.

should i follow all metrics from STEL paper? 
"""
