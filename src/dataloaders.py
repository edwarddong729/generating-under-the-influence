...
def disc_warmup_dataloaders(train_samples_per, val_samples_per, eval_samples_per, batch_size):

    train_datasets = []
    val_datasets = []
    eval_datasets = []
    
    # we'll maintain this ordering/mapping throughout
    for ind, author in enumerate(['achebe', 'baldwin', 'jackson', 'oconnor', 'salinger']):
        chosen_sentences, remainder_sentences = split_n_sentences(f'../corpus/sents_{author}.txt', train_samples_per + val_samples_per + eval_samples_per)
        ...
        save_sentences(f'../corpus/nontrain_sents_{author}.txt', non_train)
    ...
    train_dataloader = DataLoader(combined_train, batch_size=batch_size, shuffle=True, collate_fn=classification_training_collate_w_corruption)
    val_dataloader = DataLoader(combined_val, batch_size=batch_size, shuffle=True, collate_fn=classification_training_collate_w_corruption)
    eval_dataloader = DataLoader(combined_eval, batch_size=batch_size, shuffle=True, collate_fn=classification_training_collate_w_corruption)
...
def gen_warmup_dataloaders(author, train_sample_num, val_sample_num, batch_size):

    chosen_sentences, _ = split_n_sentences(f'../corpus/sents_{author}.txt', train_sample_num + val_sample_num)
