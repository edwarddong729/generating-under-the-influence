...
# consider expanding batch size back... or might be okay? 
# switch to late criterion earlier
# make it not so expensive to be a bit similar to real styles
...
def load_warm_models(device, generator_style_id):
    ...
    for name, param in generator.named_parameters():
        if name.startswith('macberth.embeddings.'):
            ...
            param.requires_grad = False
        # elif name.startswith('macberth.encoder.layer.8.'):
        #     param.requires_grad = False
        # elif name.startswith('macberth.encoder.layer.9.'):
        #     param.requires_grad = False
        else:
            param.requires_grad = True
...
def main():
    ...
    # weights_for_gen_loss = torch.tensor([1., 1., 1., 1., 1., 0.1]).to(DEVICE)
    # intensified_weights_for_gen_loss = torch.tensor([1., 1., 1., 1., 1., 0.1]).to(DEVICE)
    ...
    early_criterion = torch.nn.CrossEntropyLoss(weights_for_gen_loss)
    late_criterion = torch.nn.CrossEntropyLoss(intensified_weights_for_gen_loss)

    generator.eval()
    print('Initial sample outputs:')
    with torch.no_grad():
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # for batch_num, batch in enumerate(dataloader):
            ...
            # torch.zeros_like(one_hot_sequence).to(DEVICE)
            # for seq_ind in range(one_hot_sequence.size(0)):
            #     for token_ind in range(one_hot_sequence.size(1)):
            #         padded_out_one_hots[seq_ind, token_ind, :] = one_hot_sequence[seq_ind, token_ind, :] * pad_out[seq_ind, token_ind]    
            # one_hot_sequence = torch.cat([one_hot_sequence, padded_out_one_hots.unsqueeze(1)], dim=1)
            
            assert padded_out_one_hots.requires_grad
            # print(sequence_lengths)
            # sequence_lengths = [sequence.size(1) for sequence in index_sequence]
            # padded_sequences = one_hot_sequence.squeeze()
            ...
            if batch_num == 79 or batch_num == 159:

                generator.eval()
                print(f'Mid/End of epoch {epoch} sample outputs:')
